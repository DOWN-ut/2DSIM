...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] PlayerInLobbyNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is PlayerInLobbyNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerInLobbyNetworkObject.Length > 0 && PlayerInLobbyNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerInLobbyNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerInLobbyBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiatePlayerInLobby instead, its shorter and easier to type out ;)")]
		public PlayerInLobbyBehavior InstantiatePlayerInLobbyNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerInLobbyNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerInLobbyBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlayerInLobbyBehavior>().networkObject = (PlayerInLobbyNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of PlayerInLobby
		/// </summary>
		/// <returns>
		/// A local instance of PlayerInLobbyBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerInLobby prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerInLobbyBehavior InstantiatePlayerInLobby(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (PlayerInLobbyNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: PlayerInLobby. Add them at the NetworkManager prefab.");
				return null;
			}
